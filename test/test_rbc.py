...
import gevent
import random
import core.reliablebroadcast as rbc
from rbc import reliablebroadcast, encode, decode
...
def test_rbc1(N=4, f=1, leader=None, seed=None):
...
def test_rbc2(N=4, f=1, leader=None, seed=None):
    # Crash up to f nodes
    if seed is not None: print 'SEED:', seed
    rnd = random.Random(seed)
    router_seed = rnd.random()
    ...
    gevent.joinall(threads)
    print [t.value for t in threads]
